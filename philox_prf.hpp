...
private:
    static constexpr auto result_mask =
        detail::fffmask<output_value_type,
                        std::min<size_t>(numeric_limits<output_value_type>::digits,
                                         output_word_size)>;

public:
    static constexpr output_value_type min() {
        return 0;
    }
    static constexpr output_value_type max() {
        return result_mask;
    };
...
template <size_t r>
using philox4x32_prf_r =
    philox_prf<uint_fast32_t, 32, 4, r, 0xD2511F53, 0x9E3779B9, 0xCD9E8D57, 0xBB67AE85>;

template <size_t r>
using philox2x64_prf_r =
    philox_prf<uint_fast64_t, 64, 2, r, 0xD2B74407B1CE6E93, 0x9E3779B97F4A7C15>;
template <size_t r>
