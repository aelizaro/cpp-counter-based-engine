...
template<typename UIntType, std::size_t w, std::size_t n, std::size_t r, std::size_t c, UIntType ... consts>
class philox_engine {
...
public:
    ...
    template<class Sseq> explicit philox_engine(Sseq& s) {
        seed(s);
    }
    ...
    template<class Sseq> void seed(Sseq& seed) {
        // TODO
    }
...
private:
   ...
   using counter_type = detail::uint_fast<counter_count * word_size>; // WARNING: doesn't scale for word_count > 4
    ...
    template <std::output_iterator<const result_type&> Output, std::sized_sentinel_for<Output> Sentinel>
...
using philox4x32_rc = philox_engine<std::uint_fast32_t, 32, 4, r, c, 0xD2511F53, 0x9E3779B9, 0xCD9E8D57, 0xBB67AE85>;

template<size_t r, size_t c>
using philox4x64_rc = philox_engine<std::uint_fast64_t, 64, 4, r, c, 0xD2E7470EE14C6C93, 0x9E3779B97F4A7C15, 0xCA5A826395121157, 0xBB67AE8584CAA73B>;

using philox4x32 = philox4x32_rc<10, 1>; // oops int128 is not working, need algotithm redo with reinterpret casts
