...
concept pseudo_random_function =
    requires(Prf prf, span<typename Prf::input_value_type, Prf::input_count> in,
...
public:
    ...
    static constexpr result_type default_seed =
        20111115u; // AE: Default seed should be moved to prf?
...
private:
    ...
    O operator()(O out, S sen) {
        ...
        auto c0 = get_counter();
        in_type inn;
        out = prf{}.generate(views::iota(c0, c0 + nprf) | views::transform([&](auto ctr) {
                                 inn = in;
                                 set_counter(inn, ctr);
...
public:
    ...
    bool operator==(const counter_based_engine& rhs) const {
        return in == rhs.in && ridxref() == rhs.ridxref();
    }
    bool operator!=(const counter_based_engine& rhs) const {
        return !operator==(rhs);
    }
...
private:
    ...
    void seed_internal(InRange _in) {
        ...
        for (size_t i = counter_count; i < input_count; ++i)
            in[i] = (inp == ine)
                        ? 0
                        : seed_value_type(*inp++) & in_mask; // ?? throw if *inp > in_mask??
        set_counter(in, 0);
        ridxref() = 0;
    }
};
