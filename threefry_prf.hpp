...
template <unsigned_integral UIntType, size_t w, size_t n, size_t R, UIntType ks_parity,
          int... consts>
class threefry_prf {
    ...
    static inline void round2(Uint& c0, Uint& c1) {
        c0 = (c0 + c1) & inmask;
        c1 = rotleft(c1, rotation_constants[r % 8]);
        c1 ^= c0;
    }
    template <typename Uint>
...
#define SIMPLIFY_PBOX 1
        ...
        auto c3tmp = c3;
        c0 = (c0 + c1) & inmask;
        c3 = rotleft(c1, rotation_constants[r % 8]);
        c3 ^= c0;
        c2 = (c2 + c3tmp) & inmask;
        c1 = rotleft(c3tmp, rotation_constants[8 + r % 8]);
        c1 ^= c2;
#else
        if ((r & 1) == 0) {
            c0 = (c0 + c1) & inmask;
            ...
            c1 ^= c0;
            c2 = (c2 + c3) & inmask;
            c3 = rotleft(c3, rotation_constants[8 + r % 8]);
            c3 ^= c2;
        }
        else {
            c0 = (c0 + c3) & inmask;
            c3 = rotleft(c3, rotation_constants[r % 8]);
            c3 ^= c0;
            c2 = (c2 + c1) & inmask;
            c1 = rotleft(c1, rotation_constants[8 + r % 8]);
            c1 ^= c2;
        }
#endif
    }
    ...
    template <typename Uint>
    static void keymix4(Uint& c0, Uint& c1, Uint& c2, Uint& c3, Uint kk0, Uint kk1, Uint kk2,
                        Uint kk3, unsigned r4) {
        c0 = (c0 + kk0) & inmask;
        c1 = (c1 + kk1) & inmask;
        c2 = (c2 + kk2) & inmask;
        c3 = (c3 + kk3 + r4) & inmask;
    ...
    template <typename Uint>
    static void do4(Uint& cc0, Uint& cc1, Uint& cc2, Uint& cc3, Uint k0, Uint k1, Uint k2,
                    Uint k3) {
        ...
        // loop.  If we unroll it ourselves, it's about twice as fast.
        if (R > 0)
            round4<0>(c0, c1, c2, c3);
        if (R > 1)
            round4<1>(c0, c1, c2, c3);
        if (R > 2)
            round4<2>(c0, c1, c2, c3);
        if (R > 3)
            round4<3>(c0, c1, c2, c3);
        if (R > 3)
            keymix4(c0, c1, c2, c3, k1, k2, k3, k4, 1);

        if (R > 4)
            round4<4>(c0, c1, c2, c3);
        if (R > 5)
            round4<5>(c0, c1, c2, c3);
        if (R > 6)
            round4<6>(c0, c1, c2, c3);
        if (R > 7)
            round4<7>(c0, c1, c2, c3);
        if (R > 7)
            keymix4(c0, c1, c2, c3, k2, k3, k4, k0, 2);

        if (R > 8)
            round4<8>(c0, c1, c2, c3);
        if (R > 9)
            round4<9>(c0, c1, c2, c3);
        if (R > 10)
            round4<10>(c0, c1, c2, c3);
        if (R > 11)
            round4<11>(c0, c1, c2, c3);
        if (R > 11)
            keymix4(c0, c1, c2, c3, k3, k4, k0, k1, 3);

        if (R > 12)
            round4<12>(c0, c1, c2, c3);
        if (R > 13)
            round4<13>(c0, c1, c2, c3);
        if (R > 14)
            round4<14>(c0, c1, c2, c3);
        if (R > 15)
            round4<15>(c0, c1, c2, c3);
        if (R > 15)
            keymix4(c0, c1, c2, c3, k4, k0, k1, k2, 4);

        if (R > 16)
            round4<16>(c0, c1, c2, c3);
        if (R > 17)
            round4<17>(c0, c1, c2, c3);
        if (R > 18)
            round4<18>(c0, c1, c2, c3);
        if (R > 19)
            round4<19>(c0, c1, c2, c3);
        if (R > 19)
            keymix4(c0, c1, c2, c3, k0, k1, k2, k3, 5);
...
public:
    ...
    requires ranges::sized_range<InRange>&& integral<iter_value_t<ranges::range_value_t<InRange>>>&&
        integral<iter_value_t<O>>&& indirectly_writable<O, iter_value_t<O>>
...
template <size_t r>
using threefry2x32_prf_r =
    threefry_prf<uint_least32_t, 32, 2, r, 0x1BD11BDA, 13, 15, 26, 6, 17, 29, 16, 24>;

template <size_t r>
using threefry2x64_prf_r =
    threefry_prf<uint_least64_t, 64, 2, r, 0x1BD11BDAA9FC1A22, 16, 42, 12, 31, 16, 32, 24, 21>;

template <size_t r>
using threefry4x32_prf_r = threefry_prf<uint_least32_t, 32, 4, r, 0x1BD11BDA, 10, 11, 13, 23, 6, 17,
                                        25, 18, 26, 21, 27, 5, 20, 11, 10, 20>;

template <size_t r>
using threefry4x64_prf_r = threefry_prf<uint_least64_t, 64, 4, r, 0x1BD11BDAA9FC1A22, 14, 52, 23, 5,
                                        25, 46, 58, 32, 16, 57, 40, 37, 33, 12, 22, 32>;
