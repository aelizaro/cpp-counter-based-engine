...
void doit(string name) {
    ...
    auto range_of_ptrs_into_bulkin =
        views::iota(size_t(0), Nprf) | views::transform([&bulkin](auto i) {
            auto p = &bulkin[0] + i * prf_input_count;
            *p += 1; // *MODIFIES* bulkin.  So different randoms every time!
            return p;
        });

    perf = timeit(std::chrono::seconds(5), [&rprf, &range_of_ptrs_into_bulkin]() {
        prf_output_value_type bulkout[bulkN];
        PRF{}.generate(range_of_ptrs_into_bulkin, begin(bulkout));
        rprf = accumulate(begin(bulkout), end(bulkout), rprf, bit_xor<decltype(rprf)>{});
    });
    cout << "calling " << name << " directly (" << bulkN
         << " at a time): " << (rprf == 0 ? " (zero?!) " : "");
    cout << perf.iter_per_sec() / 1e6 << " Miters/sec";
    ...
    perf = timeit(chrono::seconds(5), [&]() { r ^= engine(); });
    ...
    cout << "calling " << name << " through engine (" << bulkN
         << " at a  time): " << (r == 0 ? " (zero?!) " : "");
    cout << perf.iter_per_sec() / 1e6 << " Miters/sec";
...
#define MAPPED(prf) \
    { string(#prf), function<void(string)>(&doit<prf>) }
#define _ ,
map<string, function<void(string)>> dispatch_map = {
    //    MAPPED(uint64_t, null_prf),
    MAPPED(threefry4x64_prf), MAPPED(threefry2x64_prf),
    MAPPED(threefry4x32_prf), MAPPED(threefry2x32_prf),

    MAPPED(philox4x64_prf),   MAPPED(philox2x64_prf),
    MAPPED(philox4x32_prf),   MAPPED(philox2x32_prf),

    MAPPED(siphash_prf<4>),   MAPPED(siphash_prf<16>),
};
